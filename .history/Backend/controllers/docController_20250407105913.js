...
exports.deleteDocument = async (req, res) => {
  try {
    ...
    // Step 1: Log the action into the Historisation table
    await Historisation.create({
      action: `Document '${doc.filename}' soft-deleted.`,
      user_id: req.user?.id || 1, // fallback to a default user ID or handle unauthenticated access
    });

    // Step 2: Perform soft delete
    await doc.destroy();

    res.status(200).json({ message: 'Document soft-deleted and historised' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
