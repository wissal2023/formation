...
const ProductSidebar = ({ setProducts }) => {
   ...
   const handleLanguage = (language) => {
      ...
      filterProducts({ language: language === languageSelected ? '' : language });
   };

   // Handle price selection
   const handlePrice = (price) => {
      setPriceSelected(prevPrice => prevPrice === price ? '' : price);
      filterProducts({ price: price === priceSelected ? '' : price });
   };

   // Handle rating selection
   const handleRating = (rating) => {
      setRatingSelected(prevRating => prevRating === rating ? null : rating);
      filterProducts({ rating: rating === ratingSelected ? null : rating });
   };

   const filterProducts = ({ category, language, price, rating }) => {
